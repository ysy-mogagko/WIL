# 개요
- runtime 환경에서 값, 타입 등을 확인
- 값, 타입 등을 변경도 가능
- 상대적으로 cpu resource 사용이 커 빈번한 사용은 추천되지 않음
# 구성
## Value
### ValueOf(i any) Value
```
...
func main() {
	...
	fmt.Println("println value[type]: ", t)
	fmt.Println("reflect value[a]: ", va)
	fmt.Println("reflect value[type]: ", vt)
}

println value[a]:  45
println value[type]:  {asdf 1 {10 200}}
reflect value[a]:  45
reflect value[type]:  {asdf 1 {10 200}}
```
자체적인 사용 보다는 return 값을 활용
## Type
### TypeOf(i any) Type
```
type Type interface {
...
func main() {
	...
	fmt.Println("reflect type[a]: ", ta)
	fmt.Println("reflect type[type]: ", tt)
}

reflect type[a]:  int
reflect type[type]:  main.SampleType
```
